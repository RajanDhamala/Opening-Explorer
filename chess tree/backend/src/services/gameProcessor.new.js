...
export async function processGame(gameData, username, userId = null) {
  try {
    ...
    for (let i = 0; i < movesToProcess; i++) {
      ...
      // Determine the player color at this position
      // After white's move (odd index), it's white's position
      // After black's move (even index), it's black's position
      const positionPlayerColor = (i % 2 === 0) ? 'white' : 'black';
      
      // Only track positions for the player's color
      if (positionPlayerColor === playerColor) {
